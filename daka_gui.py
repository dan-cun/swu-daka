...
登录方式：默认人工手动登录 —— 脚本只打开 Chrome 并导航到学校统一认证页，
账号/密码/验证码全部由人输入并点击登录，脚本仅捕获登录后的 Token
（机器瞬填输入易触发统一认证风控；且登录态有效时会直接复用缓存 Token，
无需再次登录）。如需机器代填可传 --auto-login 走旧自动模式。

运行模式：
  双击 / 无参数        -> 打开图形界面
  --headless          -> 无界面模式：读取已保存凭据执行一次打卡，结果写入日志
                         （供计划任务在每天定时时无窗口运行；Token 过期时
                          会弹出 Chrome 等待人工登录，最长 5 分钟）
...
def main_gui() -> None:
    ...
    def worker_once():
        ...
        q.put("[i] 若本机登录态有效将直接复用 Token 打卡；否则打开 Chrome，"
              "由你在窗口内手动输入账号/密码/验证码并登录（脚本不代填），完成后自动继续打卡...")
        try:
